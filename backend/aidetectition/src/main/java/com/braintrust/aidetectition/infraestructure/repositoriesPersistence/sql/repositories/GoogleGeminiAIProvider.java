...
import com.braintrust.aidetectition.domain.model.DetectedSegment.SegmentType;
...
public class GoogleGeminiAIProvider implements AIDetectionProvider {
    ...
    private String buildAIDetectionPrompt(String content) {
        return String.format("""
            You are an academic AI text detection expert. Analyze the text below and classify EVERY paragraph as either AI-generated or human-written.
            ...
            INSTRUCTIONS:
            1. Split the text into logical paragraphs or meaningful chunks (minimum 20 characters each).
            2. For EACH paragraph, determine whether it was written by AI or by a human.
            3. Assign a probability score (0.0 to 1.0) where:
               - 0.0 = Definitely human-written
               - 1.0 = Definitely AI-generated
               - A paragraph is "AI" if probability >= 0.5, otherwise "HUMAN"
            4. Provide a brief reason for each classification.
            5. Calculate an overall ai_probability for the full text.
            ...
            RESPONSE FORMAT (strict JSON, no markdown):
            {
                ...
                "segments": [
                    {
                        "text": "EXACT paragraph text copied from original",
                        "probability": 0.85,
                        "type": "AI",
                        "reason": "Highly formal structure, lacks personal voice"
                    },
                    {
                        "text": "EXACT paragraph text copied from original",
                        "probability": 0.15,
                        "type": "HUMAN",
                        "reason": "Casual tone, grammatical imperfections, personal anecdote"
                    }
                ]
            }

            CRITICAL RULES:
            - Return ONLY valid JSON. No ```json blocks, no extra text.
            - "text" must be an EXACT substring copied from the original text.
            - Classify ALL meaningful paragraphs — both AI and human sections.
            - "type" must be exactly "AI" or "HUMAN".
            """, content);
    }
    ...
    private List<DetectedSegment> generateSegmentsFromAnalysis(String content, String response, BigDecimal overallProbability) {
        ...
        try {
            ...
            if (segments.isEmpty()) {
                log.info("No segments parsed from JSON response, creating fallback segments");
    ...
    private DetectedSegment extractSegment(String content, Map<String, Object> segmentData, BigDecimal defaultProbability) {
        try {
            ...
            String typeStr = (String) segmentData.get("type");
            SegmentType segmentType = "HUMAN".equalsIgnoreCase(typeStr)
                    ? SegmentType.HUMAN : SegmentType.AI;
    ...
    private List<DetectedSegment> createFallbackSegments(String content, BigDecimal probability) {
        ...
        for (int i = 0; i < sentences.length && count < 3; i += step) {
            ...
            if (sentence.length() >= 20) {
                ...
                if (startIndex >= 0) {
                    SegmentType fallbackType = probability.compareTo(new BigDecimal("0.5")) >= 0
                            ? SegmentType.AI : SegmentType.HUMAN;
