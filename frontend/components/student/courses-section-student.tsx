...
export function CoursesSectionStudent({ studentId }: StudentCoursesSectionProps) {
    ...
    return (
      <Card className="p-6 h-64 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-primary font-medium">Loading your courses...</p>
        </div>
      </Card>
    );
  }
  ...
  if (courses.length === 0) {
    return (
      <Card className="p-6 h-40 flex flex-col items-center justify-center space-y-3">
        <p className="text-muted-foreground font-semibold">No enrolled courses found.</p>
      ...
      <div className="mb-6">
        ...
        <div className="flex gap-4 mt-3 text-sm text-muted-foreground">
          ...
          <div className="flex items-center gap-1">
            ...
            <span>{courses.length} total enrollments</span>
          </div>
        </div>
      </div>
              ...
              return (
                <Link key={course.id} href={`/courses/${course.id}`} className="block">
                  ...
                  >
                    ...
                    {/* ✅ was: bg-gray-500 → removed, secondary variant handles it */}
                    {!course.active && (
                    ...
                    <div className="flex items-start gap-4">
                      <div className={`h-12 w-12 rounded-lg ${palette.bg} flex items-center justify-center shrink-0`}>
                        {/* ✅ was: text-white → palette.text (theme-aware contrast) */}
