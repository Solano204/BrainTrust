...
import { Assignment, Quiz, CourseResourceType } from '@/app/domain/entities/CourseEntities';
import { QuizView } from '@/components/quiz-view';
import { TaskView } from '@/components/task-view';
import { useQuizDetail, useQuizzesByMonth } from '@/app/presentation/hooks/calendar/quiz-hooks';
import { useTaskDetail, useTasksByMonth } from '@/app/presentation/hooks/calendar/task-hooks';
...
const { format, startOfWeek, getStartOfMonth, addMonths, subMonths, addDays, isSameDay, isSameMonth } = dateFns;
...
export function CalendarView({ userId, userType }: CalendarViewProps) {
    const [currentMonth, setCurrentMonth] = React.useState(getStartOfMonth(new Date()));
    const [activeResource, setActiveResource] = React.useState<CalendarResource | null>(null);
        ...
        return (
            <div 
                ...
                className={`flex flex-col p-2 min-h-[8rem] border border-border/50 transition-colors 
                    ${isOutsideMonth ? 'bg-gray-100/50 text-muted-foreground/60 dark:bg-gray-700/50' : 'bg-white dark:bg-gray-800'}
                    ${isToday ? 'ring-2 ring-primary border-primary/50 dark:bg-blue-900/20' : ''}
                `}
            >
                <div className={`text-sm font-semibold mb-2 ${isToday && !isOutsideMonth ? 'text-primary' : 'text-foreground'}`}>
                    {format(date, 'dd')}
                </div>
                
                {dayEvents.map((resource, index) => {
                    const { icon, color, title } = getResourceStyles(resource);
                    const displayTitle = resource.title.length > 20 
            ...
            <Card className="max-w-7xl mx-auto p-6 space-y-4">
                ...
                <div className="flex justify-between items-center border-b pb-4">
                    ...
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        ...
                        <span>{userType === 'teacher' ? 'Teacher' : 'Student'}: {userId}</span>
                    </div>
                </div>
                ...
                {isLoading && (
                    <div className="flex items-center justify-center py-8">
                        ...
                        <span className="text-muted-foreground">
                            Loading {tasksLoading ? 'tasks' : ''}{tasksLoading && quizzesLoading ? ' and ' : ''}{quizzesLoading ? 'quizzes' : ''}...
                        </span>
                    </div>
                )}
