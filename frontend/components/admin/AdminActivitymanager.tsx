...
// ============================================================
// FILE: components/ActivityManager.tsx
// Standalone Role Activities manager — separated from CatalogManager
// ============================================================
          ...
          .map((item, idx) =>
            item === '...' ? (
              ...
              <button key={item} onClick={() => onPage(item as number)}
                ...
                style={item === page ? { background: accentColor, color: 'white' } : { background: 'transparent', color: '#6b7280' }}>
                {(item as number) + 1}
              </button>
            )
          )}
...
const ACCENT = '#f97316'; // orange — same as original role-activities color
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <select
          ...
          className="flex-1 min-w-[180px] px-3 py-2 rounded-xl border border-gray-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-orange-300 text-gray-700"
        >
      ...
      {/* ── Add form ─────────────────────────────────────────── */}
      {showAdd && (
        <div className="p-4 rounded-2xl border border-dashed border-orange-200 bg-orange-50/40 flex flex-col gap-3">
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">New Role Activity</p>

          {/* Role selector — only shown when no filter is active */}
          {!filterRole && (
            <select
              value={newRoleId}
                    ...
                    <div className={`flex items-center justify-end gap-1 transition-opacity ${editingId === act.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                      {editingId === act.id ? (
                        <>
                          ...
                          <button onClick={() => setEditingId(null)} className="p-1.5 rounded-lg bg-gray-100 text-gray-500 hover:bg-gray-200 transition-all" title="Cancel">
                        ...
                        <>
                          ...
                          <button
                            ...
                            className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-600 transition-all disabled:opacity-40"
                            title="Delete"
                          >
      ...
      {!isLoading && raw && (
        <PaginationBar
          ...
          totalElements={raw.totalElements} onPage={setPage} accentColor={ACCENT}
        />
      )}
    </div>
  );
...
export function ActivityManager() {
  ...
  return (
    <>
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-orange-50/30 p-6 font-sans">
        {/* Header */}
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-1">
            <span className="w-10 h-10 rounded-2xl flex items-center justify-center" style={{ background: ACCENT + '18' }}>
              <Activity className="w-5 h-5" style={{ color: ACCENT }} />
            </span>
            <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Role Activities</h1>
          </div>
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-3xl border border-gray-100 shadow-sm p-6">
            ...
            <div className="flex items-start gap-3 p-3 rounded-xl bg-amber-50 border border-amber-200 mb-5 text-xs text-amber-700">
              ...
              <span>
                ...
                <code className="font-mono bg-amber-100 px-1 rounded">409 Conflict</code>{' '}
