...
export async function gradeQuizSubmission(
  submission: SubmissionQuiz, // Changed from submissionId: SubmissionQuiz
  ...
  if (isMockEnabled) {
    ...
    const submissionIndex = MOCK_SUBMISSION_QUIZZES.findIndex(
      (sub) => sub.id === submission.id // Use submission.id
    );

    if (submissionIndex === -1) {
      console.error(
        `MOCK: Submission with ID ${submission.id} not found for grading`
      );
    ...
    console.log(`MOCK: Graded submission ${submission.id}`);
